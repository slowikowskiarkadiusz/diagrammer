...
export class Triangle extends Polygon {
  ...
  protected get animatedVertices(): v2d[] {
    return [this.animatedV1, this.animatedV2, this.animatedV3];
  }
  ...
  public moveTo(to: v2d): void {
    let direction = this.center.subtract(to);
    let v1Diff = this.v1.subtract(this.center);
    let v2Diff = this.v2.subtract(this.center);
    let v3Diff = this.v3.subtract(this.center);
    this.v1 = v1Diff.add(direction);
    this.v2 = v2Diff.add(direction);
    this.v3 = v3Diff.add(direction);
  }

  public isPointInTriangle(point: v2d): boolean {
    let d1 = this.sign(point, this.v1, this.v2);
    let d2 = this.sign(point, this.v2, this.v3);
    let d3 = this.sign(point, this.v3, this.v1);
